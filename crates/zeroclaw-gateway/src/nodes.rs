...
pub async fn handle_ws_nodes(
    ...
    // hook), so the only gate available here is at request time.
    // Previously nodes.enabled was only used for printing the route in
    // the startup banner — a configuration with nodes.enabled=false
    // still served /ws/nodes. Reject 404 when disabled so the route
    // is effectively absent from the API surface.
